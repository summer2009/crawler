...
func refreshCookieString(newCookie string) {
	//判断newCookie的name是否已在cookie中；是：更新value；否：添加新的cookie
	//fmt.Println("cookieStr=", cookieStr)
	tmpStr := strings.Replace(cookieStr, "Path=/;", "", -1)
	...
	for _, str := range strList {
		...
		if strings.Index(str, "TC-Page-G0=") == -1 {
			if str != "" {
				strRet = strRet + str + ";"
			}

		} else {
			strRet = strRet + newCookie + ";"
		}
	}
	cookieStr = strRet
}
...
func addCsvFileToQueue(ctx *Context, csvFileName string, lineCount int) {
	if csvFileName != "" {
		...
		//csv, err := csv4g.NewWithOpts("D:\\bwb\\golang\\src\\github.com\\henrylee2cn\\pholcus\\pholcus_pkg\\text_out\\2019-03-03 073457\\微博粉丝列表__1cde7e27__好友列表\\20000-40000.csv", WeiboUser{}, csv4g.Comma(','), csv4g.LazyQuotes(true), csv4g.SkipLine(numOfStart))
		csv, err := csv4g.NewWithOpts(csvFileName, WeiboUser{}, csv4g.Comma(','), csv4g.LazyQuotes(true), csv4g.SkipLine(numOfStart))
		if err != nil {
			fmt.Printf("Error %v\n", err)
			return
		}

		if lineCount > csv.LineLen {
			lineCount = csv.LineLen
		}
...
var WeiboFans = &Spider{
	...
	RuleTree: &RuleTree{
		Root: func(ctx *Context) {
			...
			xfiles, _ := GetAllFiles(filePathName)
			fileList = xfiles

			lenOfFileList = len(xfiles)
			fmt.Println("lenOfFileList=", lenOfFileList)

			fmt.Println("curIndexOfFileList=", curIndexOfFileList)
			fmt.Println("curCsvFileName=", xfiles[curIndexOfFileList])
			...
			err = c.Run(ctxt, chromedp.Tasks{
				...
				chromedp.ActionFunc(func(ctx context.Context, h cdp.Executor) error {
					// 获取cookie
					cookies, err := network.GetCookies().Do(ctx, h)
			...
			if level == 0 {
				...
				addCsvFileToQueue(ctx, xfiles[curIndexOfFileList], 10000)
				if curIndexOfFileList < lenOfFileList {
					curIndexOfFileList = curIndexOfFileList + 1
				}
				/*
					if filePathName != "" {
						// /*
						...
						//csv, err := csv4g.NewWithOpts("D:\\bwb\\golang\\src\\github.com\\henrylee2cn\\pholcus\\pholcus_pkg\\text_out\\2019-03-03 073457\\微博粉丝列表__1cde7e27__好友列表\\20000-40000.csv", WeiboUser{}, csv4g.Comma(','), csv4g.LazyQuotes(true), csv4g.SkipLine(numOfStart))
						csv, err := csv4g.NewWithOpts(filePathName, WeiboUser{}, csv4g.Comma(','), csv4g.LazyQuotes(true), csv4g.SkipLine(numOfStart))
						if err != nil {
							fmt.Printf("Error %v\n", err)
							return
						}
		...
		Trunk: map[string]*Rule{
			"好友列表": {
				...
				ParseFunc: func(ctx *Context) {
					...
					if strCookedReturn != "" {
						fmt.Println("!!!!!!!!!!!!!!!!!!!!!!!!!!")
						fmt.Println("oldCookie=", cookieStr)
						fmt.Println("!!!!!!!!!!!!!!!!!!!!!!!!!!")
						fmt.Println("cookieRet=", strCookedReturn)

						//更新cookie string
						refreshCookieString(strCookedReturn)
						fmt.Println("!!!!!!!!!!!!!!!!!!!!!!!!!!")
						fmt.Println("newCookie=", cookieStr)
						logs.Log.Notice(strCookedReturn)
						//logs.Log.Notice(cookieStr)
						if strings.Index(strCookedReturn, "login=") != -1 {
							//cookie到期，需重新登录
							ctx.GetSpider().RequestFree()
							addCsvFileToQueue(ctx, fileList[curIndexOfFileList], 10000)
							logs.Log.Notice(fileList[curIndexOfFileList])
							if curIndexOfFileList < (lenOfFileList - 1) {
								curIndexOfFileList = curIndexOfFileList + 1
							}
						}

					}
					fmt.Println("RequestLen=", ctx.GetSpider().RequestLen())
					if ctx.GetSpider().RequestLen() < 10 {
						//判断cookie过期时间
